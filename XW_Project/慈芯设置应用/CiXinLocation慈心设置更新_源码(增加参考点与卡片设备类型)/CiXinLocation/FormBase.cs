...
namespace CiXinLocation{
    ...
    public partial class FormBase : Form  {
        ...
        /// <param name="lew">低于版本要不要提示</param>
        /// <param name="more">高于版本要不要提示</param>
        public bool USB_VersionLow(byte[] version,bool lew,bool more)
        {
            if (version == null || version.Length < 4) return false;
            int usb_ver = version[3] >> 4;
            if (VERSION == usb_ver) return false;
            else if (VERSION > usb_ver && lew)
                if (messageBoxShow(StringUtils.appVersion))
                {
                    return true;
                }  else  {
                    Application.Exit();
                }
            else if (VERSION < usb_ver && more) messageBoxShow(StringUtils.appVersionD);
            return false;
        }
        ...
        /// <returns></returns>
        public bool messageBoxShow(string msg)
        {
