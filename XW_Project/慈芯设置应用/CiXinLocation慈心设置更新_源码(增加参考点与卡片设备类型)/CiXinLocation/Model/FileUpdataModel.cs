...
namespace CiXinLocation.Model
{
    class FileUpdataModel : FromBaseModel, FileUpdataModelInterface
    {
        ...
        public void sendData(byte type,byte[] data) {
            byte[] sendBt = null;
            //if(type == 3)
            //    sendBt = new byte[5 + data.Length];
            //else 
            sendBt = new byte[4 + data.Length];
            sendBt[0] = 0xf2;
            sendBt[1] = type;
            sendBt[sendBt.Length - 1] = 0xf1;
            /*if (type == 3) 
            {
                Array.Copy(data, 0, sendBt, 2, 2);
                sendBt[4] = Channel;
                Array.Copy(data, 2, sendBt, 5, data.Length -2);
            }
            else*/
        ...
        private void sendData_BinThread() {
            ...
            while (isUpdata) {
                ...
                if (sleep200(500)) {
                    ...
                    //Console.Write(XwDataUtils.GetTimeStamp(false) + "重发了一次......\r\n");
                    //Console.Write(XwDataUtils.dataFromTimeStamp(XwDataUtils.GetTimeStamp(true)) + "重发了一次");
                    sendCount++;
                }
        ...
        private void sendData_Bin(){
            ...
            else if (drType == DrivaceType.NODE)
            {
                id_index = new byte[ID.Length + 1];
                Array.Copy(ID, 0, id_index, 0, 2);
                id_index[2] = Channel;
            }
            else if (drType == DrivaceType.LOCATION)
            {
                id_index = ID;
            }
            else 
        ...
        public void checkBinData() {
            ...
            int sendByteLength = 10 + id_indexlength;
            if(drType == DrivaceType.TAG) sendByteLength += 1;
