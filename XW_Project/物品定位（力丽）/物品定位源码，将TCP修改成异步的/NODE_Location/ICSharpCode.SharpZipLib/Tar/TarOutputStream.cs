...
namespace ICSharpCode.SharpZipLib.Tar
{
	...
	public class TarOutputStream : Stream
	{
		...
		public override void Write(byte[] buffer, int offset, int count)
		{
			...
			if (offset < 0) {
				throw new ArgumentOutOfRangeException(SerialportSample.XWUtils.nameof(offset), "Cannot be negative");
			}
			...
			if (count < 0) {
				throw new ArgumentOutOfRangeException(SerialportSample.XWUtils.nameof(count), "Cannot be negative");
			}

			if ((currBytes + count) > currSize) {
				string errorText = string.Format("request to write '{0}' bytes exceeds size in header of '{1}' bytes",
					count, this.currSize);
				throw new ArgumentOutOfRangeException(SerialportSample.XWUtils.nameof(count), errorText);
			}
