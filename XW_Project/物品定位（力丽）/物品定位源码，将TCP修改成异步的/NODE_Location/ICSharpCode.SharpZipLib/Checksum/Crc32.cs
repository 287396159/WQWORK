	...
	public sealed class Crc32 : IChecksum
	{
		...
		public void Update(byte[] buffer, int offset, int count)
		{
			...
			if (offset >= buffer.Length) {
				throw new ArgumentOutOfRangeException(SerialportSample.XWUtils.nameof(offset), "not a valid index into buffer");
			}

			if (count < 0) {
				throw new ArgumentOutOfRangeException(SerialportSample.XWUtils.nameof(count), "cannot be less than zero");
			}

			if (offset + count > buffer.Length) {
				throw new ArgumentOutOfRangeException(SerialportSample.XWUtils.nameof(count), "exceeds buffer size");
			}
