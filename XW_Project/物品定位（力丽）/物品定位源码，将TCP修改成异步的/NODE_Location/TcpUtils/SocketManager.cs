...
namespace XGCServerSocketAsyncEventArgs.TcpUtils
{
    class SocketManager
    {
        ...
        Dictionary<String,AsyncUserToken> m_DicClients; //客户端列表  
        ...
        public void Init()
        {
            ...
            m_DicClients = new Dictionary<string, AsyncUserToken>();
        ...
        public void Stop()
        {
            ...
            lock (m_DicClients) { m_DicClients.Clear(); }
        ...
        private void ProcessAccept(SocketAsyncEventArgs e)
        {
            try
            {
                ...
                if (userToken.Remote != null) {
                    userToken.IPAddress = ((IPEndPoint)(e.AcceptSocket.RemoteEndPoint)).Address;

                    Console.Write(userToken.IPAddress.ToString() + "已连接");
                    lock (m_clients) { m_clients.Add(userToken); }
                    lock (m_DicClients)
                    {
                        if (!m_DicClients.ContainsKey(userToken.Remote.ToString()))
                        {
                            m_DicClients.Add(userToken.Remote.ToString(), userToken);
                        }
                    }
                }                      
        ...
        private void CloseClientSocket(SocketAsyncEventArgs e)
        {
            ...
            lock (m_DicClients) {
                if (token.Remote != null) { m_DicClients.Remove(token.Remote.ToString()); }
            }
        ...
        public void SendAllMessage(byte[] message)
        {
        ...
        public void SendOnlineMessage(byte[] message)
        {
            for (int i = 0; i < m_clients.Count; i++)
            {
                if (m_clients[i].IsOnLineTime) SendMessage(m_clients[i], message);
            }
        }

        public void SendMessage(String ipConfig, byte[] message)
        {
            if (m_DicClients.ContainsKey(ipConfig)) 
            {
                SendMessage(m_DicClients[ipConfig], message);
            }
        }
        ...
        public void SendMessage(AsyncUserToken token, byte[] message)
        {
            ...
            try
            {
                ...
                sendArg.Dispose();////这一句很重要，没有这句，内存会一直增加
            }
            catch (Exception e)
            {
                Console.Write("SendMessage - Error:" + e.Message);
            }
        }

        public void setOnline(String ipConfig,Boolean onLine)
        {
            if (m_DicClients.ContainsKey(ipConfig))
            {
                m_DicClients[ipConfig].IsOnLineTime = onLine;
            }
        }
