...
namespace CiXinLocation.CiXinInterface
{
    interface ComDataInterface
    {
        ...
        void sendData(byte[] buf,int index,int length);
        void sendData(byte[] buf, int index, int length,string ipInfo);
        void sendMesg(byte[] buf, TCPSendType tcpSend);
    ...
    public interface ComReadDataInterface
    {
        ...
        /// </summary>
        /// <param name="buf"></param>
        void revePortsData(byte[] buf);
        void revePortsData(byte[] buf, string ipInfo);
        void revePortsData(byte[] buf,string ipInfo,CommunicationMode comMode); 
    }
}
