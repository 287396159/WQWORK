...
using PrecisePosition.UWB_Date;
namespace PrecisePosition
{
    public partial class Form1 : Form
    {
        ...
        public UwbSendDataUtils uwbData;
        ...
        public ConcurrentDictionary<string, CardImg> sendCardImgs = new ConcurrentDictionary<string, CardImg>();
        //用于保存记录
        ...
        public void setUWB1()
        {
            int uwb1Port = loadUWBPort(Ini.NetKey_UWB1Port);
            String uwb1IP = loadUWBIP(Ini.NetKey_UWB1IP);
            uwbData.UWB1point = new IPEndPoint(IPAddress.Parse(uwb1IP), uwb1Port);
        }

        public void setUWB3()
        {
            int uwb3Port = loadUWBPort(Ini.NetKey_UWB3Port);
            String uwb3IP = loadUWBIP(Ini.NetKey_UWB3IP);
            uwbData.UWB3point = new IPEndPoint(IPAddress.Parse(uwb3IP), uwb3Port);
        }


        private int loadUWBPort(String kry)
        {
        ...
        private String loadUWBIP(String kry)
        {
        ...
        private void sendLocationJson(CardImg tag) 
        {
            if (tag.curpostype == PortType.SingleMode) return;
            UWBbean uwbBean = new UWBbean();
            uwbBean.DataTime = getTimeFormat();
            uwbBean.Method = "UWB";
            Param param = new Param(tag.ID[0].ToString("X2") + tag.ID[1].ToString("X2"), (int)tag.CardPoint.x, (int)tag.CardPoint.y,0, "T3");
            uwbBean.MParam = param;
            uwbData.sendUWB3Data(uwbBean);
            //String msg = JSON.stringify(uwbBean);
        }

        /**
         *发送单点资讯 
         */
        private void sendSingleLocationJson(String tagID,int x,int y,int dis)
        {
            UWBbean uwbBean = new UWBbean();
            uwbBean.DataTime = getTimeFormat();
            uwbBean.Method = "UWB";
            Param param = new Param(tagID, x, y, dis, "T1");
            uwbBean.MParam = param;
            uwbData.sendUWB1Data(uwbBean);
        }

        private String getTimeFormat()
        {
            return getTimeFormat(DateTime.Now);
        }

        private String getTimeFormat(DateTime dt)
        {
        ...
        private void Start_Listen_Btn_Click(object sender, EventArgs e)
        {
            ...
            else 
            {
                ...
                try
                {
                    PrecisePositionLibrary.PrecisePosition.Start(this.Handle, CurReportMode, PosititionMode.UnKnown, (Parameter.isUse3Station ? AfewDPos.Pos2Dim : AfewDPos.Pos3Dim));
                }
        ...
        private void setSortPort(ref SortPort[] CurSortPort, CardImg tag, ref string StrReferID) 
        {
            for (int i = 0; i < CurSortPort.Length; i++)
            {
                CurSortPort[i] = new SortPort();
            }
            if (tag.ReportRouters.Count >= 1)
            {
                System.Buffer.BlockCopy(tag.ReportRouters[0].id, 0, CurSortPort[0].ID, 0, 2);
                StrReferID = tag.ReportRouters[0].id[0].ToString("X2") + tag.ReportRouters[0].id[1].ToString("X2");
                CurSortPort[0].Name = Ini.GetValue(Ini.PortPath, StrReferID, Ini.Name);
                CurSortPort[0].isOptimal = true;
                CurSortPort[0].Distanse = tag.ReportRouters[0].dis;
                CurSortPort[0].SigQuality = tag.ReportRouters[0].SigQuality;
            }

            if (tag.ReportRouters.Count >= 2)
            {
                System.Buffer.BlockCopy(tag.ReportRouters[1].id, 0, CurSortPort[1].ID, 0, 2);
                StrReferID = tag.ReportRouters[1].id[0].ToString("X2") + tag.ReportRouters[1].id[1].ToString("X2");
                CurSortPort[1].Name = Ini.GetValue(Ini.PortPath, StrReferID, Ini.Name);
                CurSortPort[1].isOptimal = true;
                CurSortPort[1].Distanse = tag.ReportRouters[1].dis;
                CurSortPort[1].SigQuality = tag.ReportRouters[1].SigQuality;
            }
            if (tag.ReportRouters.Count >= 3)
            {
                System.Buffer.BlockCopy(tag.ReportRouters[2].id, 0, CurSortPort[2].ID, 0, 2);
                StrReferID = tag.ReportRouters[2].id[0].ToString("X2") + tag.ReportRouters[2].id[1].ToString("X2");
                CurSortPort[2].Name = Ini.GetValue(Ini.PortPath, StrReferID, Ini.Name);
                CurSortPort[2].isOptimal = true;
                CurSortPort[2].Distanse = tag.ReportRouters[2].dis;
                CurSortPort[2].SigQuality = tag.ReportRouters[2].SigQuality;
            }
            if (tag.ReportRouters.Count >= 4)
            {
                System.Buffer.BlockCopy(tag.ReportRouters[3].id, 0, CurSortPort[3].ID, 0, 2);
                StrReferID = tag.ReportRouters[3].id[0].ToString("X2") + tag.ReportRouters[3].id[1].ToString("X2");
                CurSortPort[3].Name = Ini.GetValue(Ini.PortPath, StrReferID, Ini.Name);
                CurSortPort[3].isOptimal = false;
                CurSortPort[3].Distanse = tag.ReportRouters[3].dis;
                CurSortPort[3].SigQuality = tag.ReportRouters[3].SigQuality;
            }
            if (tag.ReportRouters.Count >= 5)
            {
                System.Buffer.BlockCopy(tag.ReportRouters[4].id, 0, CurSortPort[4].ID, 0, 2);
                StrReferID = tag.ReportRouters[4].id[0].ToString("X2") + tag.ReportRouters[4].id[1].ToString("X2");
                CurSortPort[4].Name = Ini.GetValue(Ini.PortPath, StrReferID, Ini.Name);
                CurSortPort[4].isOptimal = false;
                CurSortPort[4].Distanse = tag.ReportRouters[4].dis;
                CurSortPort[4].SigQuality = tag.ReportRouters[4].SigQuality;
            }
        }
        ...
        public void MyTimer_Tick(object sender, EventArgs e)
        {
            ...
            foreach(KeyValuePair<string,CardImg> tag in CardImgs)
            {
                ...
                CardImg cardImg = tag.Value;
                setSortPort(ref CurSortPort, cardImg, ref StrReferID);
        ...
        public void DrawGuidsLine(Graphics g,CardImg tag)
        {
            ...
            BsInfo bs = null;
            BsInfo selebs = null;
            List<ReportRouterInfor> tagreportrouters = tag.ReportRouters.ToList<ReportRouterInfor>();
            try
            {
                for (int i = 0; i < tagreportrouters.Count; i++)
                {
                    ...
                    if (null != bs)
                    {
                        ...
                        if (Parameter.isSupportMulArea)
                        {
                            if (tag.ReportRouters[i].ResidualValue == 0)
                            {//说明这个点是未选择的点
                        ...
                        {
                            if (tag.ReportRouters[i].SigQuality >= 4)
                            {//说明这个点是未选择的点
                                DrawGuidRound(g, pt, dis, false, group);
                            }
                            else
        ...
        private void StartList_button_Click(object sender, EventArgs e)
        {
            ...
            if (!isStart)
            {
                ...
                try
                {
                    if (PrecisePositionLibrary.PrecisePosition.Start(this.Handle, CurReportMode, Parameter.positionmode, (Parameter.isUse3Station ? AfewDPos.Pos2Dim : AfewDPos.Pos3Dim)))
                    {                      
                        //开启定时器,设置刷新线程的时间间隔
                        ...
                        //将标志位置为真
                        isUpdate = true;
                        uwbData.Client = PrecisePositionLibrary.PrecisePosition.client;
                ...
                {
                    MessageBox.Show("对不起，请求的地址无效!" + se.ToString());
                }
                catch (Exception)
                {
                }
        ...
        public void DrawCard(CardImg card, Graphics g)
        {
            ...
            {
                float x1 = -1, y1 = -1, x2 = -1, y2 = -1, x3 = -1, y3 = -1, x4 = -1, y4 = -1, x5 = -1, y5 = -1;
            ...
            if (isSendIndexUWBJSON(StrID, card))
            {
                sendLocationJson(card);
            }
        ...
        private void DrawTagToPortNear(CardImg card, PosPortinfor minport, Graphics g)
        {
            ...
            SortPort[] CurSortPort = new SortPort[5];
            String StrReferID = "";
            setSortPort(ref CurSortPort, card, ref StrReferID);

            if (isSendUWBJSON(StrID, card))
            {
                sendSingleLocationJson(StrID, (int)card.CardPoint.x, (int)card.CardPoint.y, (int)Math.Round(CurSortPort[0].Distanse, 2));
            }
        ...
        /// <summary>
        /// 判断是否发送UWB json数据，若是此时发送数据时间大于（不能等于）上次发送时间
        /// </summary>
        /// <param name="StrID"></param>
        /// <param name="card"></param>
        /// <returns></returns>
        private bool isSendUWBJSON(String StrID, CardImg card) 
        {
            if (sendCardImgs.ContainsKey(StrID))
            {
                CardImg sendImg;
                bool isValue = sendCardImgs.TryGetValue(StrID, out sendImg);
                //下面if，后面的判断就是关于时间，即该包时间是否大于上一次发送的时间
                int timeCa = sendImg.ReceiTime.CompareTo(card.ReceiTime);
                if (isValue && timeCa < 0)
                {
                    sendImg.ReceiTime = card.ReceiTime;
                    sendImg.Index = card.Index;
                    return true;
                }
            }
            else
            {
                CardImg addCardImg = new CardImg();
                addCardImg.Name = card.Name;
                addCardImg.ReceiTime = card.ReceiTime;
                addCardImg.Index = card.Index;
                sendCardImgs.TryAdd(StrID, addCardImg);
                return true;
            }
            return false;
        }

        private bool isSendIndexUWBJSON(String StrID, CardImg card)
        {
            if (sendCardImgs.ContainsKey(StrID))
            {
                CardImg sendImg;
                bool isValue = sendCardImgs.TryGetValue(StrID, out sendImg);
                if (sendImg.Index != card.Index)
                {
                    sendImg.Index = card.Index;
                    return true;
                }
    ...
    public class Ini
    {
        ...
        public const String NetKey_jsonIP = "JSONIP";
        public const String NetKey_Port = "PORT";
        public const String NetKey_UWB1IP = "uwb1IP";
        public const String NetKey_UWB1Port = "uwb1PORT";
        public const String NetKey_UWB3IP = "uwb3IP";
        public const String NetKey_UWB3Port = "uwb3PORT";
        ...
        //得到Segment区中的所有Key值
        public static bool GetAllKey(String FileStr, String segment, out List<string> lists)
        {
            lists = new List<string>();
            if (!Open(FileStr, true))
            {
            ...
            byte[] bytes = new byte[len];
            if (MyFileStream != null) MyFileStream.Read(bytes, 0, len);
            if (!Close(FileStr))
            {
                return false;
            }
