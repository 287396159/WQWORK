...
namespace CiXinLocation.Model
{
    public class FromBaseModel {
        ...
        public delegate void SendDataHandle(SendDataType sType);
        ...
        public void addCSAndsendData(SendDataType sType)
        {
            ...
            if (sType.Buf == null || sType.Buf.Length < 2) return;
        ...
        {
            if (sType.isNoDt) revePortsData(sType.Buf);
        }
        ...
        private void findPack(byte[] buf){            
            for (int i = 0; i < buf.Length;i++ ) {
                ...
                for (int j = i; j < buf.Length; j++) {
                    ...
                    if (checkPacket(packData)) Array.Clear(buf,i,j-i);
                }
            }         
        }
        ...
        public virtual void reveData(byte[] buf, string ip) { }
